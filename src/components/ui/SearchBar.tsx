	...
	const search = useCallback(async (q: string) => {
		...
		try {
			const res = await fetch(
				`/api/events/search?q=${encodeURIComponent(q)}&limit=8`,
			);
			if (res.ok) {
	...
	return (
		<div ref={containerRef} className="relative w-full max-w-xl mx-auto">
			<div className="relative">
				<svg
					className={`absolute top-1/2 -translate-y-1/2 left-3 w-4 h-4 text-zinc-500`}
					fill="none"
					viewBox="0 0 24 24"
					stroke="currentColor"
					strokeWidth={2}
				...
				<input
					...
					placeholder={isFr ? "Rechercher..." : "Search events..."}
					className={`w-full bg-zinc-900/80 border border-zinc-800 rounded-lg py-2.5 pl-10 pr-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/20 transition-colors`}
