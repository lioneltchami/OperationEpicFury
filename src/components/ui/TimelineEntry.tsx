...
export const TimelineEntry = ({ event }: { event: TimelineEvent }) => {
	...
	const handleShare = async () => {
		const url = `${SITE_URL}/${locale}/events/${event.slug || event.id}`;
		if (navigator.share) {
			try {
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							{showShare && event.slug && (
								<>
									<a
										href={`https://x.com/intent/tweet?text=${encodeURIComponent(headline)}&url=${encodeURIComponent(`${SITE_URL}/${locale}/events/${event.slug}`)}`}
										target="_blank"
										rel="noopener noreferrer"
									...
									<a
										href={`https://t.me/share/url?url=${encodeURIComponent(`${SITE_URL}/${locale}/events/${event.slug}`)}&text=${encodeURIComponent(headline)}`}
										target="_blank"
										rel="noopener noreferrer"
										className="text-zinc-600 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-red-500/50 focus-visible:outline-none rounded-sm"
									...
									<a
										href={`https://wa.me/?text=${encodeURIComponent(headline + " " + `${SITE_URL}/${locale}/events/${event.slug}`)}`}
										target="_blank"
										rel="noopener noreferrer"
										className="text-zinc-600 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-red-500/50 focus-visible:outline-none rounded-sm"
