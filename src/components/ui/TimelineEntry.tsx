        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {showShare && event.slug && (
              <>
                <a href={`https://x.com/intent/tweet?text=${encodeURIComponent(headline)}&url=${encodeURIComponent(`${SITE_URL}/${locale}/events/${event.slug}`)}`} target="_blank" rel="noopener noreferrer" className="text-zinc-600 hover:text-white transition-colors" aria-label="Share on X">
                  <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <a href={`https://t.me/share/url?url=${encodeURIComponent(`${SITE_URL}/${locale}/events/${event.slug}`)}&text=${encodeURIComponent(headline)}`} target="_blank" rel="noopener noreferrer" className="text-zinc-600 hover:text-white transition-colors" aria-label="Share on Telegram">
                ...
                <a href={`https://wa.me/?text=${encodeURIComponent(headline + " " + `${SITE_URL}/${locale}/events/${event.slug}`)}`} target="_blank" rel="noopener noreferrer" className="text-zinc-600 hover:text-white transition-colors" aria-label="Share on WhatsApp">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${SITE_URL}/${locale}/events/${event.slug}`);
                    setCopied(true);
