...
const categories: { value: EventCategory; label: string; color: string }[] = [
  ...
  { value: "breaking-important", label: "Breaking Important", color: "text-red-400" },
];
...
export function AdminEventForm({ event, mode, onSaved, onCancel }: Props) {
        ...
        <div className="rounded-xl border border-zinc-800/80 bg-zinc-950/50 overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-3">
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 text-[11px] font-medium rounded-md badge-${category}`}>
  ...
  // ─── Main Form ───
  return (
    <form onSubmit={handlePreview} className="max-w-2xl space-y-6 animate-slide-up">
      {error && (
              ...
              >
                ...
                <div className="aspect-video bg-zinc-800 flex items-center justify-center relative">
                  {infoIdx === idx ? (
                    ...
                    >
                      <p><span className="text-zinc-500">type:</span> {item.type}</p>
                      {item.fileId && <p className="break-all"><span className="text-zinc-500">fileId:</span> {item.fileId}</p>}
                      {item.url && <p className="break-all"><span className="text-zinc-500">url:</span> {item.url}</p>}
                      {item.thumbnailFileId && <p className="break-all"><span className="text-zinc-500">thumb:</span> {item.thumbnailFileId}</p>}
                      {item.width && <p><span className="text-zinc-500">size:</span> {item.width}x{item.height}</p>}
                      {item.duration != null && <p><span className="text-zinc-500">duration:</span> {item.duration}s</p>}
                      {item.mimeType && <p><span className="text-zinc-500">mime:</span> {item.mimeType}</p>}
                    </div>
                  ) : item.type === "video" ? (
      ...
      <div className="rounded-xl border border-zinc-800/80 overflow-hidden">
        ...
        {showFrench && (
          <div className="px-5 py-4 space-y-4 border-t border-zinc-800/50 bg-zinc-950/30">
            ...
            <div>
              <FormLabel>Headline (FA)</FormLabel>
              <input
                className={inputClass}
                value={headlineFr}
