...
export function validateEventInput(
  body: unknown,
): { valid: true; data: Omit<TimelineEvent, "id" | "slug"> } | { valid: false; error: string } {
  ...
  if (typeof b.category !== "string" || !VALID_CATEGORIES.includes(b.category as EventCategory)) {
  ...
  if (b.source.length > MAX_SOURCE) {
    return { valid: false, error: `source must be ${MAX_SOURCE} characters or less` };
  }
  ...
  if (b.sourceUrl.length > MAX_SOURCE_URL) {
    return { valid: false, error: `sourceUrl must be ${MAX_SOURCE_URL} characters or less` };
  }
...
export function validateEventUpdate(
  body: unknown,
): { valid: true; data: Partial<TimelineEvent> } | { valid: false; error: string } {
  if (!body || typeof body !== "object") {
    return { valid: false, error: "Request body must be a JSON object" };
  }
  ...
  if ("source" in b) {
    if (typeof b.source !== "string" || b.source.length > MAX_SOURCE) {
      return { valid: false, error: `source must be a string of ${MAX_SOURCE} characters or less` };
    }
    data.source = b.source;
  }

  if ("sourceUrl" in b) {
    if (typeof b.sourceUrl !== "string" || b.sourceUrl.length > MAX_SOURCE_URL) {
      return { valid: false, error: `sourceUrl must be a string of ${MAX_SOURCE_URL} characters or less` };
    }
...
function validateMedia(items: unknown[]): MediaItem[] {
  return items
    ...
    .map((item) => {
      ...
      if (typeof item.thumbnailFileId === "string") media.thumbnailFileId = item.thumbnailFileId;
