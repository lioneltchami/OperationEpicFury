...
export async function getPublishedEventsPaginated(
	...
	// Get total count for approximate total
	const totalIds = await redis.zcard(INDEX_KEY);
	if (totalIds === 0) return { events: [], total: 0 };

	const published: TimelineEvent[] = [];
	let cursor = 0;
	const batchSize = (offset + limit) * 2; // Over-fetch to handle draft filtering

	while (published.length < offset + limit && cursor < totalIds) {
	...
	return {
		...
		total: published.length >= offset + limit ? totalIds : published.length,
	};
}
